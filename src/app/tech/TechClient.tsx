...
const specs = [
  {
    ...
    items: [
      { label: "Transport", value: "Bluetooth Low Energy 5.0+ (CoreBluetooth)" },
  ...
  },
  {
    category: "Encryption",
    items: [
      { label: "DM key agreement", value: "Noise XX handshake (Curve25519 + ChaChaPoly + SHA256)" },
      { label: "Group messages", value: "AES-256-GCM sender keys with automatic rotation" },
      { label: "Packet signing", value: "Ed25519 via libsodium (64-byte detached signatures)" },
      { label: "Signable data", value: "Full packet excluding TTL byte and signature" },
      { label: "Relay compatibility", value: "TTL excluded from signed data — relay nodes decrement without breaking sig" },
      { label: "Key distribution", value: "Public keys exchanged via auth service and announce payloads" },
      { label: "Session management", value: "Noise session per peer pair, rekeying on reconnect" },
      { label: "Key storage", value: "iOS Keychain (kSecAttrAccessibleAfterFirstUnlock)" },
    ],
  },
  {
    category: "Authentication",
    items: [
      { label: "Sign-up", value: "Email + social login (Apple, Google) — no SMS, no phone numbers" },
      { label: "Registration", value: "Ed25519 challenge-response (signed nonce)" },
      { label: "Sessions", value: "HS256 JWT, refreshed before expiry via /v1/auth/refresh" },
      { label: "Public keys", value: "Uploaded per user to auth service for DM discovery" },
      { label: "Transport", value: "TLS with certificate pinning" },
    ],
  },
  {
    category: "WebSocket Relay",
    items: [
      { label: "Purpose", value: "Off-mesh delivery when BLE peers aren't directly reachable" },
      { label: "Infrastructure", value: "Cloudflare Workers + Durable Objects" },
      { label: "Store-and-forward", value: "50 packets/peer cap, 1hr TTL" },
      { label: "Auth", value: "JWT bearer, sender PeerID verified against connection" },
      { label: "Drain", value: "Per-peer serialized delivery with retry (3 attempts, 5s×N backoff)" },
      { label: "Payload privacy", value: "Relay cannot read DM payloads — they remain E2E encrypted end-to-end" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Crowd modes", value: "Gather (<500) / Event (500–5K) / Mega (5K–25K) / Massive (25K+)" },
  ...
  },
  {
    category: "Platform",
    items: [
      { label: "iOS", value: "17.0+ (SwiftUI, SwiftData, MVVM — no third-party UI libraries)" },
      { label: "Packages", value: "BlipProtocol, BlipMesh, BlipCrypto (Swift Package Manager)" },
      { label: "Dependencies", value: "CryptoKit (Apple), swift-sodium, swift-opus, Sentry" },
      { label: "Backend", value: "Cloudflare Workers (auth, relay, CDN) + Neon Postgres + R2" },
        ...
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <p className="text-lg text-[var(--muted)] max-w-2xl">
              ...
              routing, and Ed25519 packet authentication. iOS 17+, native
              SwiftUI client, open source on GitHub.
            </p>
          </motion.div>
